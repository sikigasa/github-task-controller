...
import type { Task, TaskStatus, Priority, TaskFilters, SortConfig, FilterPreset } from '@/types';
...
export function filterTasks(
  tasks: Task[],
  filters: TaskFilters,
  options?: { alwaysShowCompleted?: boolean; assignee?: string; preset?: FilterPreset }
): Task[] {
  const { statusFilter, priorityFilter, startDate, endDate, showCompleted } = filters;
  const { alwaysShowCompleted = false, assignee, preset } = options || {};

  return tasks.filter(t => {
    // Preset filters
    if (preset === 'my-tasks-all' && t.assignee !== 'Me') return false;
    if (preset === 'my-tasks-nodate' && (t.assignee !== 'Me' || t.due)) return false;
    ...
    // Completed Filter
    if (!showCompleted && !alwaysShowCompleted && t.status === 'Done') return false;
