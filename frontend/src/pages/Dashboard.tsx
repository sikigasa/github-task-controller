...
export const Dashboard: React.FC<DashboardProps> = ({ tasks, onStatusChange, onTaskClick, onAddTask }) => {
  ...
  const getAgeInfo = (createdAt?: string) => {
    ...
    if (days < 7)
      return { label: `${days}d`, color: "text-blue-600 bg-blue-50" };
    if (days < 14)
      return { label: `${days}d`, color: "text-yellow-600 bg-yellow-50" };
    if (days < 30)
      return { label: `${days}d`, color: "text-orange-600 bg-orange-50" };
    if (days < 60)
      return { label: `${days}d`, color: "text-red-600 bg-red-50 font-medium" };
                    ...
                    <div className="flex items-center items-end text-xs text-muted-foreground mt-2">
                      {task.due ? (
                        ...
                        ageInfo && (
                          <div
                            className={cn(
                              "flex items-center px-1.5 py-0.5 rounded -ml-1.5",
                              ageInfo.color
                            )}
                          >
